...
def generate_launch_description():
    ...
    config_file_path = PathJoinSubstitution([pkg_project_gazebo, 'config', 'sst_ros_gz_bridge.yaml'])
    ...
    twist_stamper = Node(
        ...
        name='twist_stamper',
        parameters=[{
            'frame_id': 'sst/base_link/dvl_sensor',
            'input_topic': '/sst/dvl/velocity_notStamped',
            'output_topic': '/sst/dvl/velocity'
        }],
        output='screen'
    )
    ...
    depth_sensor_from_odom = Node(
        ...
        name='depth_sensor_from_odom',
        parameters=[{
            'input_topic': 'odometry',
            'output_topic': 'sst/depth_sensor/odom',
            'noise_stddev': 0.005
        }],
        output='screen'
    )
